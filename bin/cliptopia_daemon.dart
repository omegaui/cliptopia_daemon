...
void main(List<String> arguments) async {
  ...
  if(ArgumentHandler.shouldStart()){
    ...
    stdout.writeln("No Operation Provided.");
    daemon.status();
  }
}
...
void logUsage({describe = true}) {
  if(describe) {
    ...
    stdout.writeln("Cache is located at: ${Platform
        .environment['HOME']}/.config/cliptopia/cache");
    stdout.writeln();
  }
  ...
  stdout.writeln("\t--reset-cache   Stops the Daemon and clears the clipboard cache (not-recommended)");
  stdout.writeln("\t--version       Prints program version");
  stdout.writeln("\t--help          Prints this help message");
  stdout.writeln();
  stdout.writeln("and optionally:");
  stdout.writeln("\t--debug         Enable printing logs to the terminal (should only be used when debugging)");
  if(describe) {
    stdout.writeln();
